...
const WarRoom = ({ onLogout }) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (rmsError) {
        console.error('Supabase error:', rmsError)
        throw rmsError
      }
      ...
      const mappedRMs = (rmsData || []).map(rm => ({
        ...
        monthlyGap: rm.monthly_gap || 0,
        monthlyAchievement: rm.monthly_achievement || 0,
        ...
        escalationLevel: rm.escalation_level,
        lastActivity: rm.last_activity || new Date().toISOString().split('T')[0]
      }))
      
      let rmsToUse = mappedRMs
    ...
    <div style={styles.container}>
        ...
        <div>
          <div style={styles.weeklyHeader}>
            <div style={styles.monthInfo}>
              ...
              <span style={{ marginLeft: '15px', opacity: 0.9 }}>Week {currentWeek} of 4 | Monthly Progress: {metrics.revenueConversion || 0}%</span>
            </div>
            <div>
              <span style={styles.weekBadge}>📍 Week {currentWeek} Monitoring Active</span>
            </div>
          </div>

          <div style={styles.statsBar}>
            <div style={styles.stat}>
              <span style={styles.criticalDot}></span>
              <span>Critical: {criticalCount}</span>
            </div>
            <div style={styles.stat}>
              <span style={styles.highDot}></span>
              <span>High: {highCount}</span>
            </div>
            <div style={styles.stat}>
              <span style={styles.mediumDot}></span>
              <span>Medium: {mediumCount}</span>
            </div>
            <div style={styles.stat}>
              <span>✅</span>
              <span>On Track: {rms.length - (criticalCount + highCount + mediumCount)}</span>
            </div>
          </div>

          <div style={styles.meetingStatsGrid}>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px', fontWeight: 'bold' }}>{totalMeetings}</div>
              <div style={{ fontSize: '12px', opacity: 0.9 }}>Total Meetings</div>
            </div>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px', fontWeight: 'bold' }}>{totalUpcoming}</div>
              <div style={{ fontSize: '12px', opacity: 0.9 }}>Upcoming Meetings</div>
            </div>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px', fontWeight: 'bold' }}>{metrics.revenueConversion || 0}%</div>
              <div style={{ fontSize: '12px', opacity: 0.9 }}>Achievement</div>
            </div>
          </div>
      ...
      {activeTab === 'meetings' && (
        <div>
          <div style={styles.meetingStatsGrid}>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px' }}>{totalMeetings}</div>
              <div>Total Meetings</div>
            </div>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px' }}>{totalUpcoming}</div>
              <div>Upcoming</div>
            </div>
            <div style={styles.meetingStatCard}>
              <div style={{ fontSize: '28px' }}>{Object.keys(meetingStats).length}</div>
              <div>RMs</div>
            </div>
          </div>
